...
from typing import Literal # userType ke liye zaruri
...
def register(user: schemas.UserRegister, db: Session = Depends(get_db)):
    """API Endpoint: POST /api/register/ - Naya user register karta hai."""
...
# -------------------- LOGIN API (UPDATED) --------------------
# Frontend ke anusaar path: POST /api/login/{userType}
UserRole = Literal["farmer", "expert", "admin"] 
...
def login(
    ...
    """
    📝 API Endpoint: POST /api/login/{userType}
    - Login handle karta hai aur token + user object return karta hai.
    """
    
    # 1. Database me user verify karna
    db_user = crud.get_user_by_email(db, email=user.email)
    ...
    # 2. JWT token generate karna
    access_token = security.create_access_token(data={"sub": db_user.email, "role": user_type})
    
    # 3. Frontend ke liye user data format karna
    user_out = {
        "id": getattr(db_user, 'id', None),
        "name": getattr(db_user, 'name', 'Guest'), 
        "role": user_type,    
        "email": db_user.email
    }
    
    # 4. Final response return karna
    return {
        "token": access_token, 
        "user": user_out
    }
